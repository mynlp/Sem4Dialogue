...
from exec_eval import exec_on_db, result_eq, exec_on_db_
import os
...
def judge(args: Tuple[Dict[str, Any], str, bool]) -> bool:
    ...
    for testcase_path in testsuite_paths:
        ...
        try:
            flg, gold_result = exec_on_db_(sqlite_path=testcase_path, query=gold_query)
        except func_timeout.exceptions.FunctionTimedOut:
            pass_all_testcase = False
            print('execute gold programm timeout, program:'+gold_query)
        try:
            if flg != 'result':
                ...
                pass
        try:
            flg, pred_result = exec_on_db_(testcase_path, pred)
            if flg != 'result':
                pass_all_testcase = False
                break
            ...
            pass_all_testcase = False
            print('execute prediction programm timeout, program:'+pred)
...
def main(preds: List[str], gold_file: str = "classical_test.pkl", verbose: bool = True,
    ...
    # f = open('../GeographySQL/geo.txt', 'w')
    # for example in gold_dicts:
    #     f.write(example['text']+' ||| '+example['query'])
    #     f.write('\n')
    # f.close()
    ...
    result = []
    for i in tqdm(range(len(gold_dicts))):
        result.append(judge((gold_dicts[i], preds[i], use_cache)))
        #result = list(tqdm.tqdm(pool.imap(judge, zip(gold_dicts, preds, repeat(use_cache, len(preds)))), total=len(gold_dicts)))
    #result=judge((gold_dicts,preds,use_cache))
    print(result)
    if verbose:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gold', dest='gold', type=str, default='/Users/chenbowen/Documents/PaperCode/Seq2SeqParingandDialogue/sql_eval/classical_test.pkl',
                        help="the path to the predicted queries")
    parser.add_argument('--pred', dest='pred', type=str, help="the path to the predicted queries",default='/Users/chenbowen/Documents/PaperCode/Seq2SeqParingandDialogue/ATIS_SQL_Version/Transformer_pred_10000.txt')
    parser.add_argument('--out_file', type=str, help='the output file path', default='/Users/chenbowen/Documents/PaperCode/Seq2SeqParingandDialogue/sql_eval/out/all_eval_results.json')
    parser.add_argument('--num_processes', default=1, help='number of processes to use')
    parser.add_argument('--subset', default='atis', choices=('atis', 'advising', 'academic', 'imdb', 'restaurants', 'geography', 'scholar', 'yelp', 'full'),
